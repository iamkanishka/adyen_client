defmodule AdyenEx.ClientTest do
  use ExUnit.Case, async: true

  alias AdyenEx.Client

  setup do
    bypass = Bypass.open()
    config = AdyenEx.Config.load!(%{api_key: "test_key", max_retries: 2, retry_delay: 10})
  ...
  describe "retry logic" do
    ...
    test "does not retry on 400", %{bypass: bypass, config: config} do
      ...
      assert {:error, %AdyenEx.Error{type: :api_error}} =
    ...
    test "exhausts retries and returns final error", %{bypass: bypass} do
      config = AdyenEx.Config.load!(%{api_key: "k", max_retries: 2, retry_delay: 5})
      ...
      assert {:error, %AdyenEx.Error{type: :server_error}} =
