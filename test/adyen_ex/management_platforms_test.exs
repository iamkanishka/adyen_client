defmodule AdyenEx.Management.CompaniesTest do
  ...
  test "get/2 sends GET to /companies/:id", %{bypass: bypass, config: config} do
    ...
    assert {:ok, %{"id" => ^company_id}} = AdyenEx.Client.get(url, config: config)
  end
end

defmodule AdyenEx.Management.MerchantsTest do
  use ExUnit.Case, async: true
  ...
  test "create/2 sends POST to /merchants", %{bypass: bypass, config: config} do
    ...
    assert {:ok, %{"id" => "MERCHANT_NEW"}} =
             AdyenEx.Client.post(url, %{legalEntityId: "LE123"}, config: config)
  end
...
defmodule AdyenEx.Management.StoresTest do
  use ExUnit.Case, async: true
  ...
  test "create/3 sends POST to /merchants/:id/stores", %{bypass: bypass, config: config} do
    ...
    assert {:ok, %{"id" => "ST_NEW"}} =
             AdyenEx.Client.post(url, %{shopperStatement: "My Store"}, config: config)
  end
  ...
  test "update/4 sends PATCH to /merchants/:id/stores/:store_id", %{
    ...
    assert {:ok, _} = AdyenEx.Client.patch(url, %{status: "inactive"}, config: config)
  end
end

defmodule AdyenEx.BinLookupTest do
  use ExUnit.Case, async: true

  setup do
    bypass = Bypass.open()
    %{bypass: bypass, config: AdyenEx.Config.load!(%{api_key: "k"})}
  end
  ...
  test "get_cost_estimate/2 posts to /getCostEstimate", %{bypass: bypass, config: config} do
    ...
    assert {:ok, %{"costEstimateAmount" => _}} =
             AdyenEx.Client.post(url, %{merchantAccount: "M"}, config: config)
  end
end

defmodule AdyenEx.DisputesTest do
  use ExUnit.Case, async: true
  ...
  for {action, path} <- [
    ...
    test "#{action}/2 sends POST to /#{path}", %{bypass: bypass, config: config} do
      ...
      assert {:ok, _} = AdyenEx.Client.post(url, %{disputePspReference: "D123"}, config: config)
    end
  end
end

defmodule AdyenEx.TransfersTest do
  use ExUnit.Case, async: true
...
defmodule AdyenEx.CapitalTest do
  use ExUnit.Case, async: true
  ...
  test "request_grant/2 POSTs to /grants", %{bypass: bypass, config: config} do
    ...
    assert {:ok, %{"id" => "GRANT_1"}} =
             AdyenEx.Client.post(url, %{grantOfferId: "OFFER_1"}, config: config)
  end
end

defmodule AdyenEx.ClassicPaymentsTest do
  use ExUnit.Case, async: true
...
defmodule AdyenEx.LegalEntityTest do
  use ExUnit.Case, async: true
  ...
  test "create/2 POSTs to /legalEntities", %{bypass: bypass, config: config} do
    ...
    assert {:ok, %{"id" => "LE_1"}} =
             AdyenEx.Client.post(url, %{type: "individual"}, config: config)
  end
  ...
  test "get/2 GETs /legalEntities/:id", %{bypass: bypass, config: config} do
    ...
    assert {:ok, %{"id" => "LE_1"}} = AdyenEx.Client.get(url, config: config)
  end
  ...
  test "update/3 PATCHes /legalEntities/:id", %{bypass: bypass, config: config} do
    ...
    url = "http://localhost:#{bypass.port}/lem/v4/legalEntities/LE_1"
    assert {:ok, _} = AdyenEx.Client.patch(url, %{}, config: config)
  end
  ...
  test "upload_document/2 POSTs to /documents", %{bypass: bypass, config: config} do
    ...
    assert {:ok, %{"id" => "DOC_1"}} =
             AdyenEx.Client.post(url, %{type: "PASSPORT"}, config: config)
  end
end

defmodule AdyenEx.BalancePlatformTest do
  use ExUnit.Case, async: true
  ...
  test "create_account_holder/2 POSTs to /accountHolders", %{bypass: bypass, config: config} do
    ...
    assert {:ok, %{"id" => "AH_1"}} =
             AdyenEx.Client.post(url, %{legalEntityId: "LE_1"}, config: config)
  end
  ...
  test "create_balance_account/2 POSTs to /balanceAccounts", %{bypass: bypass, config: config} do
    ...
    assert {:ok, %{"id" => "BA_1"}} =
             AdyenEx.Client.post(url, %{accountHolderId: "AH_1"}, config: config)
  end
  ...
  test "create_sweep/3 POSTs to /balanceAccounts/:id/sweeps", %{bypass: bypass, config: config} do
    ...
    assert {:ok, %{"id" => "SWP_1"}} =
             AdyenEx.Client.post(url, %{schedule: %{type: "daily"}}, config: config)
  end
  ...
  test "create_payment_instrument/2 POSTs to /paymentInstruments", %{
    ...
    assert {:ok, %{"id" => "PI_1"}} =
             AdyenEx.Client.post(url, %{type: "card", balanceAccountId: "BA_1"}, config: config)
  end
  ...
  test "create_transaction_rule/2 POSTs to /transactionRules", %{bypass: bypass, config: config} do
    ...
    assert {:ok, _} = AdyenEx.Client.post(url, %{type: "velocity"}, config: config)
  end
