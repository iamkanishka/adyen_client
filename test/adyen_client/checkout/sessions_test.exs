defmodule AdyenClient.Checkout.SessionsTest do
  use ExUnit.Case, async: true

  alias AdyenClient.Checkout.Sessions
  alias AdyenClient.TestSupport.Factory

  setup do
    bypass = Bypass.open()
    config = AdyenClient.Config.load!(%{api_key: "test_key", environment: :test})
  ...
  describe "create/2" do
    test "sends POST to /sessions and returns body on 200", %{bypass: bypass} do
      ...
      result = AdyenClient.Client.post(url, params, config: config)
    ...
    test "sends x-api-key header", %{bypass: bypass} do
      ...
      config = AdyenClient.Config.load!(%{api_key: "test_key", environment: :test})
      url = "http://localhost:#{bypass.port}/v72/sessions"
      AdyenClient.Client.post(url, Factory.build(:session_params), config: config)
    end
    ...
    test "sends idempotency-key header on POST", %{bypass: bypass} do
      ...
      config = AdyenClient.Config.load!(%{api_key: "test_key"})
      url = "http://localhost:#{bypass.port}/v72/sessions"
      AdyenClient.Client.post(url, Factory.build(:session_params), config: config)
    end
    ...
    test "returns structured error on 401", %{bypass: bypass} do
      ...
      config = AdyenClient.Config.load!(%{api_key: "bad_key"})
      url = "http://localhost:#{bypass.port}/v72/sessions"
      result = AdyenClient.Client.post(url, %{}, config: config)

      assert {:error, %AdyenClient.Error{type: :auth_error, status: 401}} = result
    end
    ...
    test "returns not_found error on 404", %{bypass: bypass} do
      ...
      config = AdyenClient.Config.load!(%{api_key: "k"})
      url = "http://localhost:#{bypass.port}/v72/sessions"

      assert {:error, %AdyenClient.Error{type: :not_found}} =
               AdyenClient.Client.post(url, %{}, config: config)
    end
    ...
    test "returns validation error on 422", %{bypass: bypass} do
      ...
      assert {:error, %AdyenClient.Error{type: :validation_error, error_code: "130"}} =
               AdyenClient.Client.post(url, %{}, config: config)
    end
  end
  ...
  describe "get/3" do
    test "sends GET to /sessions/:id", %{bypass: bypass} do
      ...
      assert {:ok, body} = AdyenClient.Client.get(url, config: config)
      assert body["id"] == "CS6DB..."
    end
...
defmodule AdyenClient.Checkout.ModificationsTest do
  use ExUnit.Case, async: true
  ...
  defp base_config, do: AdyenClient.Config.load!(%{api_key: "test_key"})
  ...
  describe "cancel/3" do
    test "POSTs to /payments/:psp/cancels", %{bypass: bypass} do
      ...
      assert {:ok, _} =
               AdyenClient.Client.post(url, %{merchantAccount: "Merchant"}, config: base_config())
    end
  end
  ...
  describe "reverse/3" do
    test "POSTs to /payments/:psp/reversals", %{bypass: bypass} do
      ...
      assert {:ok, _} =
               AdyenClient.Client.post(url, %{merchantAccount: "M"}, config: base_config())
    end
  end
